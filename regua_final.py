...
def busca_largura(estado_inicial, n):
    ...
    while fila:
        ...
        movimentos = movimentos_possiveis(estado_atual, posicao_vazia, n)  # Movimentos possíveis
        num_filhos = 0
        for movimento in movimentos:
            ...
            if movimento_str not in visitados:
                ...
                if estado_meta(movimento, n):
                    nos_com_filhos += 1 if num_filhos > 0 else 0
                    filhos_totais += num_filhos
                    return movimento, profundidade + 1, caminho + [movimento], len(fila), nos_expandidos, filhos_totais / nos_com_filhos if nos_com_filhos > 0 else 0
                fila.append((movimento, profundidade + 1, caminho + [movimento])) 
                visitados.add(movimento_str)  
        if num_filhos > 0:
            nos_com_filhos += 1
            filhos_totais += num_filhos

    return None, -1, [], len(fila), nos_expandidos, filhos_totais / nos_com_filhos if nos_com_filhos > 0 else 0
...
inicio1 = time.time()
tracemalloc.start()
resultado, profundidade, passos, fronteira, nos_expandidos, fator_ramificacao = busca_largura(estado, n)
fim1 = time.time()
...
print("Número de passos:", len(passos))
print("Nós na fronteira:", fronteira)
print("Nós expandidos:", nos_expandidos)
print("Fator de ramificação médio:", round(fator_ramificacao, 2))
